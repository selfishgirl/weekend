...
#有些页面控件是js在页面加载后生成的
#implicitily_wait用来判断整个网页是否加载完毕
#有时页面加载完成，但js的控件还没创建好，所以需要加time.sleep(),提高程序的稳定性
time.sleep(2)
#driver.find_element_by_id("rt_rt_1bvoutd1v1nf2178a13lph021mpg8").send_keys("D:/upload.png")
#driver.find_element_by_css_selector("#filePicker label").send_keys("D:/upload.png")
driver.find_element_by_class_name("webuploader-element-invisible").send_keys("D:/upload.png")
#真正负责上传文件的页面元素是<input type="file">,可以直接操作这个控件，这个控件可以直接输入图片路径
#查看方法点击选择图片右键，event listeners--click--label的js文件label.on('click',function())
...
#range是区间，默认为从0开始，到长度-1结束，(10)表示0到9，十个数
